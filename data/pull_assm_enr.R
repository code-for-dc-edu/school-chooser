...
school_names <- lapply(profiles, function(x){
    sprintf("%04d %s", x$code, x$org_name)
})

school_names <- unlist(school_names)


for(i in 1:length(assmessment_data)){
    filename <- school_names[i] %+% ".csv"
